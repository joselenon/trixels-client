...
interface ResourcesCooldownProps {
  [resourceType: string]: { cooldown: number };
}
...
export default function AddNewTimer({
  ...
  const { addUserResource } = useTimersFunctions();
  ...
  const handleAddNewTimer = () => {
    ...
    /* MELHORAR NOME DE VARIAVEIS (RESOURCETYPE E resourceType) */
    for (const key of resourcesListKeys) {
      if (
        resourcesList[key].landNumber === landNumber &&
        resourcesList[key].resourceType === resourceType
      ) {
